...
const NewsDetailModify = (props) => {
    ...
    return(
        <>
        {/* <KeyEvent event1={props.closeNewsDetailModify}/> */}
        {props.isNewsDetilModifyOpen && (
            ...
            <div className="bg-gray-300 w-full h-5/6 my-auto mx-auto rounded shadow-lg rounded-2xl z-50 relative p-10">
                    <div className="modal-header pb-3 flex justify-between">
                                <input id="title" className="text-2xl font-semibold w-full" defaultValue={props.newsDetailItem.title}></input>
                            </div>
                            <div className="modal-body bg-white h-5/6 pt-5 pl-2 rounded-3xl">
                                <textarea id="content" className="w-full h-80 bg-white" defaultValue={props.newsDetailItem.content}></textarea>
                            ...
                            <div className="modal-footer pt-3 flex justify-between">
                                <button className="modal-close px-4 py-2 bg-blue-500 hover:bg-blue-700 text-white rounded absolute bottom-2 left-4" onClick={()=>{                                
                                    ...
                                    try{
                                        axios.post(url+"/updateNews",{id:props.newsDetailItem.id, title:document.getElementById("title").value, content:document.getElementById("content").value}).then(result=>{
                                            if(result.data === undefined) {
                                                alert("데이터가 없다!!");
                                            }
                                            else{
                                                setNews({id:"", title:"", content:"", attached:"", created_time:""});
                                                props.toggleUpdatePage();
                                                props.closeNewsDetailModify();
                                            }
                                        })
                                    }catch(e){
                                        console.log("에러다! : ", e);
                                ...
                                <button className="modal-close px-4 py-2 bg-blue-500 hover:bg-blue-700 text-white rounded absolute bottom-2 right-4" onClick={props.closeNewsDetailModify}>
                                    닫기
                                </button>
                            </div>
                        </div>
...
export default NewsDetailModify;